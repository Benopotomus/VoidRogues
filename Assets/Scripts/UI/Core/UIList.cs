...
namespace VoidRogues.UI
{
        ...
        where RContent : MonoBehaviour
    {
        ...
        public int Hovered { get { return _hovered; } set { SetHovered(value, false); } }
        ...
        private void SetHovered(int hovered, bool notify, bool force = false)
        {
            if (_allowSelection == false)
                return;

            if (hovered >= _dataCount)
                return;

            if (hovered == _hovered && force == false)
                return;
            ...
            {
                var item = _items[i];
                item.IsHovered = item.ID == _hovered;
            }

            if (notify == true)
            {
                HoveredChanged?.Invoke(_hovered);
            }
        }
        ...
        {
            if (id == _hovered && _allowDeselection == false)
                return;

            SetHovered(id == _hovered ? -1 : id, true);
        }
    }
}
